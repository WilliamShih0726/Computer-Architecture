...
uint64_t hash_kmer(const char* s, int k) {
    uint64_t packed = 0;
    for(int i=0; i<k; ++i) {
        packed = (packed << 8) | ((uint8_t)s[i]);
    }
    // Accelerator
    uint64_t result = 0;
    asm volatile(
        "accelerator_reduce_sum %[s], %[a], %[b]\n"
        : [s]"=r"(result)
        : [a]"r"(packed), [b]"r"(0)
    );
    return result;
...
void generate_index(const char **index_sequences, int index_sequences_count, int k, HashMap *map){
    ...
    size_t cap = 1;
    while (cap < (size_t)(total / 0.7)) cap <<= 1;
    ...
    for (int tid = 0; tid < index_sequences_count; ++tid) {
        ...
        for (int j = 0; j <= L - k; ++j) {
            strncpy(kmer, &s[j], k);
            kmer[k] = '\0';
            // 用 custom accelerator_reduce_sum hash k-mer 字串
            uint64_t key = hash_kmer(kmer, k);
    ...
    for (size_t i = 0; i < cap; ++i) {
        ...
        // 反解回 k-mer 字串 (因用hash值，這裡只是形式，實際查找還是靠hash值)
        for (int p = k - 1; p >= 0; --p) {
            kmer[p] = LUT[ky & 3];
            ky >>= 2;
        }
