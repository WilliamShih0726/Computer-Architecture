...
struct Bucket {
    size_t total_len;
    int    idx;
    bool operator>(const Bucket& o) const { return total_len > o.total_len; }
};
...
{
    // 內部 lambda：對一組序列做「最大先分配 + 最小負載優先」平衡
    auto balance_by_length = [logic_unit_count](std::vector<std::string>& seqs, std::vector<std::vector<std::string>>& partitions)
    {
        // 1. 長度由大到小排序
        std::sort(seqs.begin(), seqs.end(),
                  [](const std::string& a, const std::string& b){
                      return a.size() > b.size();   // > 由大到小 < 由小到大
                  });

        // 2. 用 min-heap 追蹤每個 partitions[i] 的累積長度
        std::priority_queue<Bucket, std::vector<Bucket>, std::greater<Bucket>> pq;
        for (int i = 0; i < logic_unit_count; ++i) {
            pq.push({0, i});
        }

        // 3. 最長的序列依序丟給當前最空的Logic Unit
        for (const auto& s : seqs) {
            Bucket b = pq.top(); pq.pop();
            partitions[b.idx].push_back(s);
            b.total_len += s.size();
            pq.push(b);
        }
    };

    balance_by_length(transcript_sequences, transcript_sequence_partitions);
    balance_by_length(query_sequences, query_sequence_partitions);
}
